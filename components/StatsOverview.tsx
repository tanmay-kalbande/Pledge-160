...
export const StatsOverview: React.FC<StatsOverviewProps> = ({ user, logs, goal, onGoalClick }) => {
  ...
  const StatBox = ({ label, value, icon: Icon, colorClass, onClick, isClickable }: any) => (
    <div
      onClick={onClick}
      className={`bg-noir-surface border border-noir-border p-4 rounded-xl flex flex-col justify-between h-24 relative overflow-hidden group transition-all duration-200 shadow-lg ${isClickable ? 'cursor-pointer hover:border-gold hover:bg-noir-elevated' : ''
        }`}
    >
      <div className="absolute right-2 top-2 opacity-10 group-hover:opacity-20 transition-opacity">
        <Icon size={40} />
      </div>
      <span className="text-zinc-500 text-[10px] uppercase tracking-widest font-bold flex items-center gap-1">
        {label}
        {isClickable && <span className="w-1.5 h-1.5 rounded-full bg-gold animate-pulse"></span>}
      </span>
